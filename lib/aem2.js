...
(function (root, $) {
  ...
  var AEM = root.AEM = {
    ...
    meta: function (path, cb) {
      return this.get({
        url: this.baseURL + this.normalize(path) + '/jcr:content.json',
        dataType: 'text',
    ...
    metaField: function (path, field, cb) {
      return this.get({
        url: this.baseURL + this.normalize(path) + '/jcr:content/' + field + '.json',
        dataType: 'text',
        dataFilter: function (jcr) {
          jcr = JSON.parse(jcr)
          return jcr[field] || null
        }
      }).done(cb)
    },
