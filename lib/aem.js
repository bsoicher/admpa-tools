...
(function (root, $) {
  ...
  var AEM = root.AEM = {
    ...
    asset: function (path, cb) {
      return this.get(this.baseURL + this.normalizeAsset(path) + '/_jcr_content.json').done(cb)
    },

    // Check if asset exists
    assetExists: function (path, cb) {
      return $.Deferred(function (d) {
        AEM.asset(path).then(function () {
          d.notify(100).resolve(true)
        }, function () {
          d.notify(100).resolve(false)
        })
    ...
    get: (function () {
      ...
      function next () {
        if (stack.length !== 0 && active < get.limit) {
